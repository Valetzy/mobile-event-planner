...
if (isset($_GET['id'])) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        // Update user status
        $update_query = "UPDATE supplier SET status = 'denied' WHERE supplier_id = $user_id";
        if (mysqli_query($conn, $update_query)) {
            ...
            <body onload="Swal.fire({ icon: \'success\', title: \'Rejected\', text: \'Supplier Rejected.\' }).then(() => { window.location.href = \'../client_pending.php\'; })">
            </body>
            </html>';
        } else {
