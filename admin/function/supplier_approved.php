...
if (isset($_GET['id'])) {
    ...
    $select_query = "SELECT * FROM supplier WHERE supplier_id = $user_id";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        // Update user status
        $update_query = "UPDATE supplier SET status = 'approved' WHERE supplier_id = $user_id";
        if (mysqli_query($conn, $update_query)) {
            ...
            <body onload="Swal.fire({ icon: \'success\', title: \'Success\', text: \'Supplier Approved.\' }).then(() => { window.location.href = \'../supplier_pending.php\'; })">
            </body>
            </html>';
        } else {
